...
<template>
	<div>
		<div v-if="data.id.length>0" style="width: 100%; height: auto">
					...
					<div class="book_info">
						...
						<el-space
							...
							><span>库存：</span>
								<span
								>{{
									(data?.inventory.total ?? 0) - (data?.inventory.borrowed ?? 0)
...
async function loader(id: string): Promise<void> {
	...
	if (book === null || (book.id.localeCompare(id) !== 0)) {
		...
		const {
				  data: {data: cur}
			  } = await cacheInstance.get(`${HttpURL.getBook}${id}`);
		merge(data, cur);
	} else {
		merge(data, book);
	}
...
function commit() {
	...
	commitComment(store.user.id, route.params.id as string, commentContent.value)
		.then(
